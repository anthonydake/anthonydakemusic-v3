      ...
      <div className="relative min-h-screen bg-white text-black">
        ...
        <main className="relative z-[10] mx-auto max-w-5xl px-6 pb-24 pt-24 text-center sm:px-8 lg:px-10 xl:px-12">
          <div className="space-y-6">
            <div className="text-[10.625px] uppercase tracking-[0.28em] text-black/45">Performance</div>
            <h1 className="text-balance text-[clamp(25.5px,3.4vw,48.875px)] font-medium uppercase tracking-[0.02em] text-black">
              Performance Unavailable
            </h1>
            <p className="text-[0.9375rem] text-black/60">This performance detail isn&apos;t live yet.</p>
            <Link
              href={contactHref}
        ...
        <div className="grid h-full grid-rows-[auto_minmax(0,1fr)_auto] gap-6 lg:gap-8">
          <section aria-label="Show hero" className="w-full border-y border-black/10 bg-white px-6 py-5 text-center sm:py-6">
            <div className="mx-auto flex w-full max-w-4xl flex-col gap-2">
              <div className="text-[9.5625px] uppercase tracking-[0.28em] text-black/50">{performance.dateDisplay}</div>
              <h1 className="text-balance text-[clamp(14.875px,2.2vw,25.5px)] font-medium uppercase leading-[1.05] tracking-[0.02em] text-black">
                {performance.title}
              </h1>
                ...
                {performance.artistLinks?.length ? (
                  <div className="flex flex-wrap items-center justify-center gap-2">
                    {performance.artistLinks.map((link) => (
                      <a
                        key={link.url}
                        className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white/70 px-4 py-2 text-[10.625px] uppercase tracking-[0.24em] text-black/70 transition hover:-translate-y-0.5 hover:shadow-md"
                        href={link.url}
                        target="_blank"
                        rel="noreferrer"
                      >
                  ...
                  <div className="text-[clamp(10.625px,1.4vw,14.875px)] uppercase tracking-[0.18em] text-black/70">
                    {performance.primaryArtist}
                  </div>
                )}
              </div>
              {performance.venue || locationLine ? (
                <div className="flex items-center justify-center">
                  {mapHref ? (
                    <a
                      className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white/70 px-4 py-2 text-[9.5625px] uppercase tracking-[0.22em] text-black/70 transition hover:-translate-y-0.5 hover:shadow-md"
                      href={mapHref}
                      target="_blank"
                      rel="noreferrer"
                    >
                    ...
                    <div className="text-[9.5625px] uppercase tracking-[0.22em] text-black/55">{locationLine}</div>
                  )}
                </div>
              ) : null}
            </div>
          ...
          <section aria-label="Show content" className="grid min-h-0 gap-6">
            <div className="flex min-h-0 flex-col">
              {embedUrl ? (
                ...
                <div className="aspect-[4/3] w-full overflow-hidden rounded-2xl border border-black/10 bg-white px-4 py-4 text-center text-[9.5625px] uppercase tracking-[0.28em] text-black/55">
